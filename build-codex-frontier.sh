...
OUTPUT="$APP_HOME/dist/Codex-Frontier-2.6.0.apk"
...
apksigner sign \
  ...
  --out "$WORK_DIR/Codex-Frontier-2.6.0.apk" \
  "$WORK_DIR/aligned.apk"

zipalign -c 4 "$WORK_DIR/Codex-Frontier-2.6.0.apk"
apksigner verify --verbose --print-certs "$WORK_DIR/Codex-Frontier-2.6.0.apk" >/dev/null
aapt dump badging "$WORK_DIR/Codex-Frontier-2.6.0.apk" | grep -q "package: name='$EXPECTED_PACKAGE' versionCode='9' versionName='2.6.0'"
cp "$WORK_DIR/Codex-Frontier-2.6.0.apk" "$OUTPUT"
if [ -f "$WORK_DIR/Codex-Frontier-2.6.0.apk.idsig" ]; then
  cp "$WORK_DIR/Codex-Frontier-2.6.0.apk.idsig" "$OUTPUT.idsig"
fi
