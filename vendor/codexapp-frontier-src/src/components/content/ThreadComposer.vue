<template>
  <form class="thread-composer" @submit.prevent="onSubmit(isTurnInProgress ? activeInProgressMode : 'steer')">
    ...
    <p v-if="commandFeedback" class="thread-composer-command-feedback" role="status">
      {{ commandFeedback }}
    </p>
    <p
      v-if="selectionEvidenceLabel"
      class="thread-composer-command-feedback"
      :class="{ 'is-error': selectionEvidence?.status === 'mismatch' }"
      role="status"
    >
      {{ selectionEvidenceLabel }}
    </p>

    <section v-if="threadGoal" class="thread-composer-goal" :class="`is-${threadGoal.status}`">
      <div class="thread-composer-goal-copy">
        <span class="thread-composer-goal-kicker">GOAL · {{ formatGoalStatus(threadGoal.status) }}</span>
        <strong class="thread-composer-goal-objective">{{ threadGoal.objective }}</strong>
        <span class="thread-composer-goal-meta">{{ formatGoalUsage(threadGoal) }}</span>
      </div>
      <div class="thread-composer-goal-actions">
        <button
          ...
          type="button"
          :disabled="isGoalBusy"
          @click="updateGoalStatus('active')"
        >Resume</button>
        <button
          v-else-if="threadGoal.status === 'active'"
          type="button"
          :disabled="isGoalBusy"
          @click="updateGoalStatus('paused')"
        >Pause</button>
        <button type="button" :disabled="isGoalBusy" @click="beginGoalEdit">Edit</button>
        <button type="button" :disabled="isGoalBusy" @click="removeGoal">Clear</button>
      </div>
    </section>
    ...
    >
      ...
      >
        ...
        <div v-if="isSlashMenuOpen" class="thread-composer-slash-menu" role="listbox" aria-label="Codex commands">
          <button
          ...
          >
            <span class="thread-composer-slash-name">{{ command.name }}</span>
            <span class="thread-composer-slash-description">{{ command.description }}</span>
          </button>
          <div v-if="filteredSlashCommands.length === 0" class="thread-composer-file-mention-empty">
...
const props = defineProps<{
  ...
  selectionEvidence?: UiTurnSelectionEvidence | null
...
const SLASH_COMMANDS: SlashCommand[] = [
  { name: '/goal', usage: '/goal <objective>', description: 'Create, inspect, pause, resume, edit, or clear a durable goal' },
  { name: '/plan', usage: '/plan [task]', description: 'Enable plan mode, optionally sending a task' },
  { name: '/fast', usage: '/fast', description: 'Toggle fast mode when the selected model supports it' },
  { name: '/compact', usage: '/compact', description: 'Compact the current thread context' },
  { name: '/fork', usage: '/fork', description: 'Fork the current thread into an independent copy' },
  { name: '/new', usage: '/new', description: 'Start a new thread' },
  { name: '/skills', usage: '/skills', description: 'Open the skills workspace' },
  { name: '/plugins', usage: '/plugins', description: 'Open installed plugins and MCP tools' },
  { name: '/apps', usage: '/apps', description: 'Open connected apps' },
  { name: '/mcp', usage: '/mcp', description: 'Open MCP server and plugin controls' },
  { name: '/automations', usage: '/automations', description: 'Open scheduled automations' },
  { name: '/model', usage: '/model', description: 'Show the active model and model controls' },
  { name: '/status', usage: '/status', description: 'Show model, reasoning, speed, and goal status' },
  { name: '/review', usage: '/review <scope>', description: 'Send a focused code-review request' },
  { name: '/permissions', usage: '/permissions', description: 'Explain the active approval and sandbox controls' },
]
...
const isRealThread = computed(() => {
  const threadId = props.activeThreadId.trim()
  return threadId.length > 0 && threadId !== '__new-thread__'
})
...
const isSlashMenuOpen = computed(() => {
  const value = draft.value.trimStart()
  if (slashDismissed.value || !value.startsWith('/') || value.includes('\n')) return false
...
const reasoningOptions = computed(() =>
  props.reasoningEfforts.map((effort) => ({ value: effort, label: REASONING_LABELS[effort] })),
)
...
const selectionEvidenceLabel = computed(() => {
  const evidence = props.selectionEvidence
  if (!evidence) return ''
  if (evidence.status === 'mismatch') {
    return `Selection mismatch · requested ${formatModelLabel(evidence.modelId)} / ${REASONING_LABELS[evidence.reasoningEffort]} · accepted ${formatModelLabel(evidence.acceptedModelId || 'unknown')} / ${evidence.acceptedReasoningEffort ? REASONING_LABELS[evidence.acceptedReasoningEffort] : 'Unknown'}`
  }
  const prefix = evidence.status === 'accepted' ? 'Server verified' : 'Locked for this turn'
  return `${prefix} · ${formatModelLabel(evidence.modelId)} · ${REASONING_LABELS[evidence.reasoningEffort]}`
})
...
const canSubmit = computed(() => {
  ...
  if (isGoalBusy.value) return false
...
function formatGoalUsage(goal: ThreadGoal): string {
  ...
  if (goal.tokenBudget !== null) {
    parts.push(`${goal.tokensUsed.toLocaleString()} / ${goal.tokenBudget.toLocaleString()} tokens`)
  ...
  if (goal.timeUsedSeconds > 0) {
    ...
    parts.push(minutes > 0 ? `${minutes}m elapsed` : `${goal.timeUsedSeconds}s elapsed`)
  }
  return parts.length > 0 ? parts.join(' · ') : 'Durable across turns'
}

async function refreshThreadGoal(): Promise<void> {
  if (!isRealThread.value) {
    threadGoal.value = null
    return
  }
  try {
    threadGoal.value = await getThreadGoal(props.activeThreadId)
  } catch (error) {
    threadGoal.value = null
    commandFeedback.value = error instanceof Error ? error.message : 'Could not read this thread goal.'
  }
}

async function updateGoalStatus(status: ThreadGoalStatus): Promise<void> {
  if (!isRealThread.value) return
  isGoalBusy.value = true
  commandFeedback.value = ''
  try {
    threadGoal.value = await setThreadGoal(props.activeThreadId, { status })
    commandFeedback.value = `Goal ${formatGoalStatus(status).toLowerCase()}.`
  } catch (error) {
    commandFeedback.value = error instanceof Error ? error.message : 'Could not update the goal.'
  } finally {
    isGoalBusy.value = false
  }
}

function beginGoalEdit(): void {
  const objective = threadGoal.value?.objective.trim()
  if (!objective) return
  draft.value = `/goal edit ${objective}`
...
async function removeGoal(): Promise<void> {
  ...
  commandFeedback.value = ''
  try {
    await clearThreadGoal(props.activeThreadId)
    threadGoal.value = null
    commandFeedback.value = 'Goal cleared.'
  } catch (error) {
    commandFeedback.value = error instanceof Error ? error.message : 'Could not clear the goal.'
  } finally {
    isGoalBusy.value = false
  }
}

function clearComposerAfterSubmit(): void {
...
function clearCommandText(): void {
  draft.value = ''
  slashDismissed.value = false
  clearPersistedDraftForThread(props.activeThreadId)
  closeFileMention()
...
function emitMessage(text: string, mode: 'steer' | 'queue', goalObjective?: string): void {
  if (!props.selectedModel.trim() || !props.selectedReasoningEffort) {
    commandFeedback.value = 'Choose an available model and thinking level before sending.'
    return
  }
  emit('submit', {
    text,
    ...
    mode,
    modelId: props.selectedModel.trim(),
...
type SlashCommandResult = 'not-command' | 'handled' | 'submitted' | 'failed'

async function handleSlashCommand(text: string, mode: 'steer' | 'queue'): Promise<SlashCommandResult> {
  const match = text.match(/^\/(\S+)(?:\s+([\s\S]*))?$/)
  if (!match) return 'not-command'
  const command = `/${(match[1] ?? '').toLowerCase()}`
  const argument = (match[2] ?? '').trim()
  if (!SLASH_COMMANDS.some((item) => item.name === command) && command !== '/clear') {
    return 'not-command'
  }
  ...
  try {
    if (command === '/goal') {
      const goalMatch = argument.match(/^(pause|resume|clear|edit)(?:\s+([\s\S]*))?$/i)
      const action = goalMatch?.[1]?.toLowerCase() ?? ''
      const actionArgument = (goalMatch?.[2] ?? '').trim()

      if (!argument) {
        await refreshThreadGoal()
        commandFeedback.value = threadGoal.value
          ? `Goal ${formatGoalStatus(threadGoal.value.status).toLowerCase()}: ${threadGoal.value.objective}`
          : 'No goal is set. Use /goal <objective> to create one.'
        return 'handled'
      }
      if (action === 'clear') {
        if (!isRealThread.value) throw new Error('Start a thread before clearing its goal.')
        await clearThreadGoal(props.activeThreadId)
        threadGoal.value = null
        commandFeedback.value = 'Goal cleared.'
        return 'handled'
      ...
      if (action === 'pause' || action === 'resume') {
        if (!isRealThread.value) throw new Error('Start a thread before changing its goal status.')
        const status: ThreadGoalStatus = action === 'pause' ? 'paused' : 'active'
        threadGoal.value = await setThreadGoal(props.activeThreadId, { status })
        commandFeedback.value = `Goal ${action === 'pause' ? 'paused' : 'resumed'}.`
        return 'handled'
      }
      if (action === 'edit') {
        if (!isRealThread.value) throw new Error('Start a thread before editing its goal.')
        if (!actionArgument) throw new Error('Use /goal edit <objective>.')
        if (actionArgument.length > 4000) throw new Error('Goal objectives are limited to 4,000 characters.')
        threadGoal.value = await setThreadGoal(props.activeThreadId, { objective: actionArgument })
        commandFeedback.value = 'Goal objective updated.'
        return 'handled'
      }

      if (argument.length > 4000) throw new Error('Goal objectives are limited to 4,000 characters.')
      if (isRealThread.value) {
        threadGoal.value = await setThreadGoal(props.activeThreadId, { objective: argument, status: 'active' })
        emitMessage(argument, mode)
      } else {
        emitMessage(argument, mode, argument)
      }
      return 'submitted'
    }
    ...
    if (command === '/plan') {
      ...
      commandFeedback.value = 'Plan mode enabled.'
      return 'handled'
    }
    if (command === '/fast') {
      if (!isFastModeSupported.value) throw new Error('Fast mode is not supported by the selected model.')
      const nextMode: SpeedMode = props.selectedSpeedMode === 'fast' ? 'standard' : 'fast'
      emit('update:selected-speed-mode', nextMode)
      commandFeedback.value = `Fast mode ${nextMode === 'fast' ? 'enabled' : 'disabled'}.`
      return 'handled'
    }
    if (command === '/compact') {
      if (!isRealThread.value) throw new Error('Start a thread before compacting context.')
      await compactThread(props.activeThreadId)
      commandFeedback.value = 'Context compaction started.'
      return 'handled'
    }
    if (command === '/fork') {
      if (!isRealThread.value) throw new Error('Start a thread before forking it.')
      const forked = await forkThread(props.activeThreadId)
      await router.push({ name: 'thread', params: { threadId: forked.threadId } })
      commandFeedback.value = 'Forked into an independent thread.'
      return 'handled'
    }
    ...
    if (command === '/plugins' || command === '/mcp') {
      await router.push({ name: 'skills', query: { tab: 'plugins' } })
      return 'handled'
    }
    if (command === '/apps') {
      await router.push({ name: 'skills', query: { tab: 'apps' } })
      return 'handled'
    }
    ...
    if (command === '/model') {
      const verified = props.selectionEvidence?.status === 'accepted' ? ' · Server verified' : ''
      commandFeedback.value = `Model: ${props.selectedModel || 'loading'} · Thinking: ${props.selectedReasoningEffort || 'default'}${verified}. Use the controls below to change them.`
      return 'handled'
    }
    if (command === '/status') {
      const goalStatus = threadGoal.value ? ` · Goal: ${formatGoalStatus(threadGoal.value.status)}` : ' · Goal: none'
      const verified = props.selectionEvidence?.status === 'accepted' ? ' · Selection: server verified' : ' · Selection: pending next turn'
      commandFeedback.value = `Model: ${props.selectedModel || 'loading'} · Thinking: ${props.selectedReasoningEffort || 'default'} · Speed: ${props.selectedSpeedMode}${verified}${goalStatus}`
      return 'handled'
    }
    if (command === '/review') {
      const reviewScope = argument || 'Review the current workspace changes for correctness, regressions, security issues, and missing tests.'
      emitMessage(reviewScope, mode)
      return 'submitted'
    }
    if (command === '/permissions') {
      commandFeedback.value = 'Permissions and sandboxing are enforced by Codex for each thread. Goal mode never broadens them.'
      return 'handled'
    }
  } catch (error) {
    commandFeedback.value = error instanceof Error ? error.message : `Could not run ${command}.`
    return 'failed'
  }
  return 'not-command'
}
...
async function onSubmit(mode: 'steer' | 'queue' = 'steer'): Promise<void> {
  ...
  if (text.startsWith('/')) {
    isGoalBusy.value = true
    const result = await handleSlashCommand(text, mode)
    isGoalBusy.value = false
    if (result === 'handled') {
      clearCommandText()
      return
    }
    if (result === 'submitted' || result === 'failed') return
  }
  emitMessage(text, mode)
}
...
function onInputKeydown(event: KeyboardEvent): void {
  if (isSlashMenuOpen.value) {
    ...
    if (event.key === 'ArrowDown') {
      ...
      if (filteredSlashCommands.value.length > 0) {
        slashHighlightedIndex.value =
          (slashHighlightedIndex.value + 1) % filteredSlashCommands.value.length
      }
      return
    }
    if (event.key === 'ArrowUp') {
      event.preventDefault()
      if (filteredSlashCommands.value.length > 0) {
        const size = filteredSlashCommands.value.length
    ...
    if (event.key === 'Enter' || event.key === 'Tab') {
      event.preventDefault()
      const selected = filteredSlashCommands.value[slashHighlightedIndex.value]
...
function applySlashCommand(command: SlashCommand): void {
  const acceptsArgument = /<|\[/.test(command.usage)
  draft.value = `${command.name}${acceptsArgument ? ' ' : ''}`
  slashDismissed.value = true
  slashHighlightedIndex.value = 0
  closeFileMention()
  void nextTick(() => {
...
.thread-composer-command-feedback {
  @apply mb-2 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs leading-5 text-cyan-950 shadow-sm;
}

.thread-composer-goal {
  @apply mb-2 flex flex-col gap-2 rounded-2xl border border-violet-200 bg-gradient-to-r from-slate-950 via-indigo-950 to-violet-950 px-3 py-3 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between;
}

.thread-composer-goal.is-paused,
.thread-composer-goal.is-blocked,
.thread-composer-goal.is-usageLimited,
.thread-composer-goal.is-budgetLimited {
  @apply border-amber-300 from-slate-950 via-amber-950 to-orange-950;
}

.thread-composer-goal.is-complete {
  @apply border-emerald-300 from-slate-950 via-emerald-950 to-teal-950;
}

.thread-composer-goal-copy {
  @apply min-w-0 flex flex-col gap-0.5;
}

.thread-composer-goal-kicker {
  @apply text-[10px] font-bold tracking-[0.18em] text-cyan-300;
}

.thread-composer-goal-objective {
  @apply line-clamp-2 text-sm font-semibold leading-5 text-white;
}
...
.thread-composer-goal-actions button {
  @apply rounded-full border border-white/20 bg-white/10 px-2.5 py-1 text-[11px] font-medium text-white transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50;
}
...
.thread-composer-slash-menu {
  @apply absolute left-0 right-0 bottom-[calc(100%+8px)] z-50 max-h-72 overflow-y-auto rounded-2xl border border-indigo-200 bg-white p-1.5 shadow-2xl;
}

.thread-composer-slash-row {
  @apply flex w-full items-center gap-3 rounded-xl border-0 bg-transparent px-3 py-2 text-left transition hover:bg-indigo-50;
}
...
.thread-composer-slash-name {
  @apply w-24 shrink-0 font-mono text-xs font-bold text-indigo-700;
}

.thread-composer-slash-description {
  @apply min-w-0 text-xs leading-4 text-zinc-600;
}
