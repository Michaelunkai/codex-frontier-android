...
import { buildRuntimeIdentityDeveloperInstructions, withRuntimeIdentity } from '../runtimeIdentity.js'
...
function rewriteImportedStructuredCwd(value: unknown, importedCwd: string): void {
  if (Array.isArray(value)) {
    for (const item of value) rewriteImportedStructuredCwd(item, importedCwd)
    return
  }
  const record = asRecord(value)
  if (!record) return
  for (const [key, nested] of Object.entries(record)) {
    if (key === 'cwd' && typeof nested === 'string') {
      record[key] = importedCwd
    } else {
      rewriteImportedStructuredCwd(nested, importedCwd)
    }
  }
}
...
function rewriteImportedSession(raw: string, importedCwd: string, importedThreadId: string): string {
  ...
  for (const line of raw.split(/\r?\n/u)) {
    ...
    try {
      ...
      if (payload) rewriteImportedStructuredCwd(payload, importedCwd)
      if (record?.type === 'session_meta' && payload) {
        payload.id = importedThreadId
        if (typeof payload.session_id === 'string') {
          payload.session_id = importedThreadId
        }
        if (typeof payload.forked_from_id === 'string') {
          payload.imported_forked_from_id = payload.forked_from_id
...
function runStateDbSql(
  stateDbPath: string,
  sql: string,
  output: 'none' | 'pipe' | 'json' = 'none',
): { status: number; stdout: string; stderr: string } {
  const sqliteArgs = output === 'json' ? ['-json', stateDbPath, sql] : [stateDbPath, sql]
  const sqliteResult = spawnSync('sqlite3', sqliteArgs, { encoding: 'utf8' })
  if (!sqliteResult.error && sqliteResult.status === 0) {
    return { status: 0, stdout: sqliteResult.stdout ?? '', stderr: sqliteResult.stderr ?? '' }
  }
  const pythonScript = String.raw`
import json, sqlite3, sys
database_path, output_mode = sys.argv[1], sys.argv[2]
sql = sys.stdin.read()
...
try:
    if output_mode == 'none':
        ...
        if output_mode == 'json':
            ...
            print(json.dumps([dict(zip(names, row)) for row in rows]))
        else:
            for row in rows:
    ...
    connection.close()
`
  const pythonResult = spawnSync('python', ['-c', pythonScript, stateDbPath, output], {
    encoding: 'utf8',
    input: sql,
  })
  return {
    status: pythonResult.status ?? 1,
    stdout: pythonResult.stdout ?? '',
    stderr: pythonResult.stderr ?? sqliteResult.stderr ?? sqliteResult.error?.message ?? '',
  }
}
...
CREATE TABLE IF NOT EXISTS threads (
  ...
  const result = runStateDbSql(stateDbPath, sql)
  if (result.status !== 0) {
...
function registerImportedSessionsInStateDb(sessions: ImportedSessionRecord[]): void {
  ...
  const columnsResult = runStateDbSql(stateDbPath, 'PRAGMA table_info(threads);', 'pipe')
...
LIMIT 200;
`
  const result = runStateDbSql(stateDbPath, sql, 'json')
...
async function importProjectZip(buffer: Buffer, destinationParent: string): Promise<{ projectPath: string; importedSessions: number }> {
  ...
  // Register last: Codex's filesystem indexer can observe the newly written
  // rollout immediately and must not overwrite the isolated project cwd after
  // our authoritative import metadata is stored.
...
function normalizeStoredQueuedMessage(value: unknown): StoredQueuedMessage | null {
  ...
  return {
    ...
    reasoningEffort: normalizeReasoningEffort(record.reasoningEffort) || 'medium',
  }
}
...
${escapeHeartbeatXmlText(automation.prompt)}
    ...
    // Automations have no composer selection; queue execution resolves the current configured pair.
    modelId: '',
...
class AppServerProcess {
  ...
  async rpc(method: string, params: unknown): Promise<unknown> {
    ...
    const resolvedParams = method === 'turn/start'
      ? await this.withAuthoritativeRuntimeIdentity(params)
      : params
    return this.call(method, resolvedParams)
  }

  private async withAuthoritativeRuntimeIdentity(params: unknown): Promise<Record<string, unknown>> {
    const input = asRecord(params)
    if (!input) throw new Error('turn/start requires an object payload.')

    let model = readNonEmptyString(input.model)
    let effort = normalizeReasoningEffort(input.effort)
    let providerId = 'openai'
    const threadId = readNonEmptyString(input.threadId)

    if ((!model || !effort) && threadId) {
      try {
        const resumed = asRecord(await this.call('thread/resume', { threadId }))
        model ||= readNonEmptyString(resumed?.model)
        effort ||= normalizeReasoningEffort(resumed?.reasoningEffort)
        providerId = readNonEmptyString(resumed?.modelProvider) || providerId
      } catch {
        // A brand-new thread may not have a rollout yet; use the current native config below.
      }
    }

    if (!model || !effort) {
      const configPayload = asRecord(await this.call('config/read', {}))
      const config = asRecord(configPayload?.config)
      model ||= readNonEmptyString(config?.model)
      effort ||= normalizeReasoningEffort(config?.model_reasoning_effort)
    }

    if (!model || !effort) {
      throw new Error('turn/start could not resolve an authoritative model and reasoning effort.')
    }
    return withRuntimeIdentity(input, model, effort, providerId)
  }
...
export class BackendQueueProcessor {
  ...
  private async buildQueuedTurnParams(turn: BackendQueuedTurn): Promise<Record<string, unknown>> {
    ...
    const queuedModel = turn.message.modelId.trim()
    const queuedEffort = normalizeReasoningEffort(turn.message.reasoningEffort)
    const settings = queuedModel && queuedEffort
      ? { model: queuedModel, reasoningEffort: queuedEffort }
      : await this.resolveCollaborationModeSettings(turn.message.collaborationMode)

    if (!settings.model || !settings.reasoningEffort) {
      throw new Error('Queued turn has no exact model and reasoning-effort selection.')
    }
    ...
    const selectedModel = models
      ...
      .find((row) => (readNonEmptyString(row?.id) || readNonEmptyString(row?.model)) === settings.model)
    if (!selectedModel) {
      throw new Error(`Queued model ${settings.model} is no longer available.`)
    }
    const supportedEfforts = Array.isArray(selectedModel.supportedReasoningEfforts)
      ? selectedModel.supportedReasoningEfforts
        .map((option) => normalizeReasoningEffort(asRecord(option)?.reasoningEffort))
        .filter((effort): effort is ReasoningEffort => effort.length > 0)
      : []
    if (supportedEfforts.length > 0 && !supportedEfforts.includes(settings.reasoningEffort)) {
      throw new Error(`Queued effort ${settings.reasoningEffort} is not supported by ${settings.model}.`)
    }
    ...
    params.collaborationMode = {
      ...
      settings: {
        ...
        developer_instructions: buildRuntimeIdentityDeveloperInstructions(
          settings.model,
...
type SharedBridgeState = {
  ...
  completionEventStore: FrontierCompletionEventStore
}

const SHARED_BRIDGE_KEY = '__codexRemoteSharedBridge__'
const SHARED_BRIDGE_VERSION = 'experimental-api-v3-frontier-completion-events'
...
function getSharedBridgeState(): SharedBridgeState {
  ...
  if (existing) {
    ...
    existing.completionEventStore?.dispose()
  ...
  const completionEventStore = new FrontierCompletionEventStore(
    join(getCodexHomeDir(), 'run', 'frontier-completion-events.json'),
    appServer,
  )
...
export function createCodexBridgeMiddleware(): CodexBridgeMiddleware {
  const { appServer, terminalManager, methodCatalog, telegramBridge, backendQueueProcessor, completionEventStore } = getSharedBridgeState()
  ...
  const middleware = async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/codex-api/frontier-completion-events') {
        const requestedSequence = Number.parseInt(url.searchParams.get('after') ?? '0', 10)
        const after = Number.isSafeInteger(requestedSequence) && requestedSequence > 0 ? requestedSequence : 0
        setJson(res, 200, {
          data: {
            latestSequence: completionEventStore.getLatestSequence(),
            events: completionEventStore.listAfter(after),
          },
        })
        return
      }

      if (req.method === 'POST' && url.pathname === '/codex-api/threads/delete-all') {
        const payload = asRecord(await readJsonBody(req))
        const confirmation = typeof payload?.confirmation === 'string' ? payload.confirmation : ''
        if (confirmation !== DELETE_ALL_SESSIONS_CONFIRMATION) {
          setJson(res, 400, { error: `Type ${DELETE_ALL_SESSIONS_CONFIRMATION} exactly to continue` })
          return
        }
        const pendingRequests = appServer.listPendingServerRequests()
        if (pendingRequests.length > 0) {
          setJson(res, 409, { error: 'Resolve pending approval requests before deleting sessions' })
          return
        }
        try {
          const result = await purgeAllSessionsAndThreads({
            ...
            appServer,
            removeThreadAutomation: (threadId) => deleteThreadHeartbeatAutomation(threadId),
            clearCompletionHistory: () => completionEventStore.clearHistoryPreservingSequence(),
          })
          threadSearchIndex = null
          threadSearchIndexPromise = null
          setJson(res, 200, { data: result })
        } catch (error) {
          const message = getErrorMessage(error, 'Failed to delete sessions safely')
          const statusCode = message.includes('active work') ? 409 : 500
