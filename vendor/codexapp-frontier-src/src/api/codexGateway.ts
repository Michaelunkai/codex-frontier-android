...
import { buildRuntimeIdentityDeveloperInstructions } from '../runtimeIdentity'
...
export async function resumeThread(threadId: string, options: { force?: boolean } = {}): Promise<ResumedThread> {
  const existing = options.force === true ? undefined : recentResumeThreadById.get(threadId)
  ...
  const promise = (async () => {
    ...
    return {
      model: normalizeThreadModelFromPayload(payload),
      reasoningEffort: normalizeReasoningEffort((payload as { reasoningEffort?: unknown }).reasoningEffort),
      modelProvider: normalizeThreadModelProviderFromPayload(payload),
      messages,
      inProgress: readThreadInProgressFromResponse(payload),
      activeTurnId: readActiveTurnIdFromResponse(payload),
...
export async function setThreadGoal(
  ...
  if (typeof update.objective === 'string') params.objective = update.objective
  if (typeof update.status === 'string') params.status = update.status
  if (update.tokenBudget !== undefined) params.tokenBudget = update.tokenBudget
...
export async function startThread(cwd?: string, model?: string, effort?: ReasoningEffort): Promise<StartedThread> {
  try {
    ...
    const acceptedModel = normalizeThreadModelFromPayload(payload)
    const acceptedEffort = normalizeReasoningEffort((payload as { reasoningEffort?: unknown }).reasoningEffort)
    const requestedModel = model?.trim() ?? ''
    if ((requestedModel && acceptedModel !== requestedModel) || (effort && acceptedEffort !== effort)) {
      try {
        await callRpc('thread/delete', { threadId })
      } catch {
        // Preserve the authoritative mismatch error even if cleanup is unavailable.
      }
      throw new Error(
        `Codex accepted ${acceptedModel || 'unknown model'} / ${acceptedEffort || 'unknown effort'} instead of ${requestedModel || 'requested model'} / ${effort || 'requested effort'}. The thread was not used.`,
      )
    }
    return {
      threadId,
...
async function resolveCollaborationModeSettings(
  ...
  throw new Error(`${mode === 'plan' ? 'Plan' : 'Default'} mode requires an explicitly selected or configured model. Select a model and try again.`)
}
...
export async function startThreadTurn(
  ...
  try {
    ...
    if (!normalizedModel) {
      throw new Error('A concrete model must be selected before starting a turn.')
    }
    if (!effort) {
      throw new Error('A concrete reasoning effort must be selected before starting a turn.')
    }
    ...
    if (collaborationMode) {
      ...
      if (!collaborationModeSettings.reasoningEffort) {
        throw new Error('Collaboration mode requires a concrete reasoning effort.')
      }
      params.collaborationMode = {
        ...
        settings: {
          ...
          developer_instructions: buildRuntimeIdentityDeveloperInstructions(
            collaborationModeSettings.model,
...
export async function getAvailableModels(options: { includeProviderModels?: boolean; requireProviderModels?: boolean; providerId?: string } = {}): Promise<AvailableModelInfo[]> {
  ...
  const providerRows = (providerModels?.ids ?? []).map((id): AvailableModelInfo => ({
    id,
    displayName: id,
  ...
  for (const row of payload.data) {
    ...
    const normalizedDefault = normalizeReasoningEffort(row.defaultReasoningEffort)
    models.push({
      id,
      ...
      defaultReasoningEffort: normalizedDefault || supportedReasoningEfforts[0] || 'medium',
    })
  }
  ...
  for (const candidate of providerRows) {
    if (!models.some((model) => model.id === candidate.id)) models.push(candidate)
  }
  return models
}
...
export async function deleteAllSessionsAndThreads(confirmation: string): Promise<SessionPurgeResult> {
  const response = await fetch('/codex-api/threads/delete-all', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ confirmation }),
  })
  const envelope = await response.json().catch(() => ({})) as { data?: SessionPurgeResult; error?: string }
  if (!response.ok || !envelope.data) {
    throw new Error(getErrorMessageFromPayload(envelope, 'Failed to delete sessions safely'))
  }
