...
export function useDesktopState() {
  ...
  const turnSelectionEvidenceByThreadId = ref<Record<string, UiTurnSelectionEvidence>>({})
  ...
  function selectionContextIdForThread(threadId: string): string {
    const contextId = toThreadContextId(threadId)
    if (contextId !== NEW_THREAD_COLLABORATION_MODE_CONTEXT) return contextId
    return toProviderModelContextId(normalizeProviderContextId(activeProviderId.value)) || contextId
  }

  function readReasoningEffortForThread(threadId: string): ReasoningEffort | '' {
    const selectedContextId = selectionContextIdForThread(threadId)
    const contextEffort = normalizeStoredReasoningEffort(selectedReasoningEffortByContext.value[selectedContextId])
    if (contextEffort) return contextEffort
    return normalizeStoredReasoningEffort(
      selectedReasoningEffortByContext.value[NEW_THREAD_COLLABORATION_MODE_CONTEXT],
    )
  }

  function readAvailableModel(modelId: string): AvailableModelInfo | null {
  ...
  function resolveReasoningEffortForModel(
    ...
    const preferred = normalizeStoredReasoningEffort(preferredEffort)
    if (!model) return preferred || 'medium'
    if (preferred && model.supportedReasoningEfforts.includes(preferred)) return preferred
  ...
  function setSelectedReasoningEffortForThread(threadId: string, effort: ReasoningEffort): void {
    ...
    nextEffortMap[selectedContextId] = effort
    if (selectedContextId !== contextId) delete nextEffortMap[contextId]
  ...
  function reconcileReasoningEffortForThread(threadId: string, modelId: string): ReasoningEffort | '' {
    const effort = resolveReasoningEffortForModel(modelId, readReasoningEffortForThread(threadId))
    if (effort) setSelectedReasoningEffortForThread(threadId, effort)
    return effort
  }

  const availableReasoningEfforts = computed<ReasoningEffort[]>(() => {
    const model = readAvailableModel(selectedModelId.value)
    return model ? [...model.supportedReasoningEfforts] : [...REASONING_EFFORT_OPTIONS]
  })

  const selectedThreadSelectionEvidence = computed<UiTurnSelectionEvidence | null>(() => {
    const threadId = selectedThreadId.value
    return threadId ? turnSelectionEvidenceByThreadId.value[threadId] ?? null : null
  })
  ...
  function setSelectedThreadId(nextThreadId: string, options: { persist?: boolean } = {}): void {
    ...
    selectedReasoningEffort.value = resolveReasoningEffortForModel(
      ...
      readReasoningEffortForThread(nextThreadId),
    )
  ...
  function setSelectedModelIdForThread(threadId: string, modelId: string): void {
    ...
    if (toThreadContextId(threadId) === toThreadContextId(selectedThreadId.value)) {
      ...
      reconcileReasoningEffortForThread(threadId, selectedModelId.value)
    } else {
      ensureAvailableModelIds(normalizedModelId)
      reconcileReasoningEffortForThread(threadId, normalizedModelId)
    }
  ...
  function setThreadModelId(threadId: string, modelId: string): void {
    ...
    if (selectedThreadId.value === normalizedThreadId) {
      selectedModelId.value = readModelIdForThread(selectedThreadId.value)
      reconcileReasoningEffortForThread(normalizedThreadId, selectedModelId.value)
    }
    saveSelectedModelMap(selectedModelIdByContext.value)
  }
  ...
  function resolveExactTurnSelection(
    threadId: string,
    override?: { modelId: string; reasoningEffort: ReasoningEffort | '' },
  ): TurnSelection {
    const modelId = (override?.modelId ?? readModelIdForThread(threadId)).trim()
    const reasoningEffort = normalizeStoredReasoningEffort(
      override?.reasoningEffort ?? readReasoningEffortForThread(threadId),
    )
    if (!modelId) throw new Error('Select a model before sending.')
    if (!reasoningEffort) throw new Error('Select a reasoning effort before sending.')
    if (availableModelIds.value.length > 0 && !availableModelIds.value.includes(modelId)) {
      throw new Error(`The selected model ${modelId} is no longer available. Refresh the model list and choose again.`)
    }
    const model = readAvailableModel(modelId)
    if (model && !model.supportedReasoningEfforts.includes(reasoningEffort)) {
      throw new Error(`${model.displayName || modelId} does not support ${reasoningEffort} reasoning. Choose one of: ${model.supportedReasoningEfforts.join(', ')}.`)
    }
    return { modelId, reasoningEffort }
  }

  function recordRequestedTurnSelection(threadId: string, turnId: string, selection: TurnSelection): void {
    turnSelectionEvidenceByThreadId.value = {
      ...turnSelectionEvidenceByThreadId.value,
      [threadId]: {
        ...selection,
        turnId,
  ...
  function recordAcceptedThreadSelection(
    threadId: string,
    modelId: string,
    reasoningEffort: ReasoningEffort | '',
    turnId = '',
    ...
    const normalizedModelId = modelId.trim()
    if (!normalizedModelId || !reasoningEffort) return
    turnSelectionEvidenceByThreadId.value = {
      ...turnSelectionEvidenceByThreadId.value,
      [threadId]: {
        ...
        turnId,
        status: 'accepted',
        acceptedModelId: normalizedModelId,
        acceptedReasoningEffort: reasoningEffort,
      },
    }
    setThreadModelId(threadId, normalizedModelId)
    setSelectedReasoningEffortForThread(threadId, reasoningEffort)
  }

  async function verifyAcceptedTurnSelection(threadId: string): Promise<void> {
    const expected = turnSelectionEvidenceByThreadId.value[threadId]
    if (!expected || expected.status !== 'requested') return

    let lastError: unknown = null
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        const resumed = await resumeThread(threadId, { force: true })
        const acceptedModelId = resumed.model.trim()
        const acceptedReasoningEffort = resumed.reasoningEffort
        if (!acceptedModelId || !acceptedReasoningEffort) {
          throw new Error('Codex did not report the accepted model and reasoning effort.')
        }
        if (acceptedModelId !== expected.modelId || acceptedReasoningEffort !== expected.reasoningEffort) {
          turnSelectionEvidenceByThreadId.value = {
            ...turnSelectionEvidenceByThreadId.value,
            [threadId]: {
              ...expected,
              status: 'mismatch',
              acceptedModelId,
              acceptedReasoningEffort,
            },
          }
          const mismatchMessage = `Selection mismatch: requested ${expected.modelId} / ${expected.reasoningEffort}, but Codex reported ${acceptedModelId} / ${acceptedReasoningEffort}.`
          setTurnErrorForThread(threadId, mismatchMessage)
          error.value = mismatchMessage
          return
        }
        if (resumed.modelProvider) setThreadModelProviderId(threadId, resumed.modelProvider)
        recordAcceptedThreadSelection(threadId, acceptedModelId, acceptedReasoningEffort, expected.turnId)
        return
      } catch (unknownError) {
        lastError = unknownError
        if (attempt < 3 && isThreadNotFoundError(unknownError)) {
    ...
    const verificationMessage = lastError instanceof Error
      ? `Could not verify the accepted model and effort: ${lastError.message}`
      : 'Could not verify the accepted model and effort.'
    setTurnErrorForThread(threadId, verificationMessage)
    error.value = verificationMessage
  }
  ...
  async function refreshModelPreferences(options?: { providerChanged?: boolean; includeProviderModels?: boolean }): Promise<void> {
    ...
    try {
      ...
      const storedEffortBeforeRefresh = readReasoningEffortForThread(selectedThreadId.value)
      const modelCatalog = await getAvailableModels({
      ...
      const modelIds = modelCatalog.map((model) => model.id)
      ...
      const nextAvailableModels = modelCatalog.map((model) => ({
        ...model,
      ...
      if (
        ...
        nextAvailableModels.push({
          ...
          supportedReasoningEfforts: [...REASONING_EFFORT_OPTIONS],
          defaultReasoningEffort: currentConfig.reasoningEffort || 'medium',
        })
      }
      availableModelIds.value = nextModelIds
      ...
      const selectedModel = readModelIdForThread(selectedThreadId.value)
      const selectedModelInfo = readAvailableModel(selectedModel)
      const storedEffortIsSupported = Boolean(
        storedEffortBeforeRefresh
        && (!selectedModelInfo || selectedModelInfo.supportedReasoningEfforts.includes(storedEffortBeforeRefresh)),
      )
      const configuredEffort = selectedModel === normalizedConfiguredModelId
        ? currentConfig.reasoningEffort
        : ''
      const nextEffort = resolveReasoningEffortForModel(
        selectedModel,
        storedEffortIsSupported ? storedEffortBeforeRefresh : configuredEffort,
      )
      if (nextEffort) setSelectedReasoningEffortForThread(selectedThreadId.value, nextEffort)
  ...
  function pruneThreadScopedState(flatThreads: UiThread[]): void {
    ...
    const nextSelectedEffortMap = pruneThreadContextStateMap(selectedReasoningEffortByContext.value, activeThreadIds)
    if (nextSelectedEffortMap !== selectedReasoningEffortByContext.value) {
      selectedReasoningEffortByContext.value = nextSelectedEffortMap
      selectedReasoningEffort.value = resolveReasoningEffortForModel(
        selectedModelId.value,
        readReasoningEffortForThread(selectedThreadId.value),
      )
      saveSelectedReasoningEffortMap(nextSelectedEffortMap)
    }
    ...
    turnSelectionEvidenceByThreadId.value = pruneThreadStateMap(turnSelectionEvidenceByThreadId.value, activeThreadIds)
  ...
  function applyRealtimeUpdates(notification: RpcNotification): void {
    ...
    if (completedTurn) {
      ...
      void verifyAcceptedTurnSelection(completedTurn.threadId)
    }
  ...
  async function sendMessageToSelectedThread(
    ...
    selectionOverride?: { modelId: string; reasoningEffort: ReasoningEffort | '' },
  ): Promise<void> {
    ...
    const selection = resolveExactTurnSelection(threadId, selectionOverride)
  ...
  async function sendMessageToNewThread(
    ...
    const selection = resolveExactTurnSelection(NEW_THREAD_COLLABORATION_MODE_CONTEXT, selectionOverride)
    ...
    try {
      ...
      recordAcceptedThreadSelection(threadId, startedThread.model, startedThread.reasoningEffort)
      ...
      const normalizedGoalObjective = goalObjective?.trim() ?? ''
      if (normalizedGoalObjective) {
      ...
      void startTurnForThread(threadId, nextText, imageUrls, skills, fileAttachments, selectedMode, selection)
        .catch((unknownError) => {
          shouldAutoScrollOnNextAgentEvent = false
          setThreadInProgress(threadId, false)
          setTurnActivityForThread(threadId, null)
  ...
  async function startTurnForThread(
    ...
    try {
      ...
      if (startedTurnId) {
        ...
        recordRequestedTurnSelection(threadId, startedTurnId, selection)
  ...
  function steerQueuedMessage(messageId: string): void {
    ...
    setSelectedModelIdForThread(threadId, msg.modelId)
    setSelectedReasoningEffortForThread(threadId, msg.reasoningEffort)
    void sendMessageToSelectedThread(
      ...
      { modelId: msg.modelId, reasoningEffort: msg.reasoningEffort },
    )
  }
  ...
  return {
    ...
    selectedThreadSelectionEvidence,
    selectedSpeedMode,
