...
beforeEach(() => {
  ...
  gatewayMocks.getAvailableModels.mockImplementation(async (options?: unknown) => {
    const modelIds = await gatewayMocks.getAvailableModelIds(options)
    return (Array.isArray(modelIds) ? modelIds : []).map((id: string) => ({
      id,
      displayName: id,
...
describe('provider model selection', () => {
  ...
  it('captures the active provider when creating a new thread', async () => {
    ...
    expect(gatewayMocks.startThread).toHaveBeenCalledWith('/tmp/project', 'gpt-5.5', 'medium')
...
describe('exact per-turn selection', () => {
  it('restores Ultra for Sol and removes it when the user switches to Luna', async () => {
    installTestWindow()
    gatewayMocks.getThreadGroupsPage.mockResolvedValue({ groups: [], nextCursor: null })
    gatewayMocks.getAvailableCollaborationModes.mockResolvedValue([{ value: 'default', label: 'Default' }])
    gatewayMocks.getSkillsList.mockResolvedValue([])
    ...
    expect(state.availableReasoningEfforts.value).toContain('ultra')

    state.setSelectedModelIdForThread('__new-thread__', 'gpt-5.6-luna')

    expect(state.selectedReasoningEffort.value).toBe('medium')
    expect(state.availableReasoningEfforts.value).toEqual(['low', 'medium', 'high', 'xhigh', 'max'])
  })

  it('does not let resume overwrite the pair frozen at Send and verifies the accepted pair after completion', async () => {
    installTestWindow()
    let notificationHandler: ((notification: { method: string; params?: unknown }) => void) | undefined
    gatewayMocks.subscribeCodexNotifications.mockImplementation((handler) => {
      notificationHandler = handler as typeof notificationHandler
    ...
    gatewayMocks.startThreadTurn.mockResolvedValue('turn-luna-medium')

    const state = useDesktopState()
    state.primeSelectedThread('thread-existing')
    state.setSelectedModelIdForThread('thread-existing', 'gpt-5.6-luna')
    ...
    expect(state.readModelIdForThread('thread-existing')).toBe('gpt-5.6-luna')
    expect(state.selectedThreadSelectionEvidence.value).toMatchObject({
    ...
    notificationHandler?.({
      ...
      params: {
        ...
        turn: { id: 'turn-luna-medium', status: 'completed' },
      },
    })
    await Promise.resolve()
    await Promise.resolve()
    ...
    expect(gatewayMocks.resumeThread).toHaveBeenLastCalledWith('thread-existing', { force: true })
    expect(state.selectedThreadSelectionEvidence.value).toMatchObject({
      modelId: 'gpt-5.6-luna',
      reasoningEffort: 'medium',
      status: 'accepted',
      acceptedModelId: 'gpt-5.6-luna',
      acceptedReasoningEffort: 'medium',
    })
  })

  it('stores the exact pair inside queued messages', async () => {
    installTestWindow()
    gatewayMocks.resumeThread.mockResolvedValue({
      model: 'gpt-5.6-sol',
      reasoningEffort: 'ultra',
    ...
    const state = useDesktopState()
    state.primeSelectedThread('thread-queued')
    await state.loadMessages('thread-queued')
    state.setSelectedModelIdForThread('thread-queued', 'gpt-5.6-sol')
