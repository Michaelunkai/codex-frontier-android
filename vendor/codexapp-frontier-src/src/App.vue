        ...
        >
          <Transition name="settings-panel">
            ...
            >
              ...
              <section class="sidebar-session-danger" aria-labelledby="delete-all-sessions-title">
                <div class="sidebar-session-danger-copy">
                  <strong id="delete-all-sessions-title">{{ t('Clean sessions and threads') }}</strong>
                  <span>{{ t('Deletes every chat session and thread. Projects, project files, plugins, skills, accounts, settings, models, memories, and project automations stay intact.') }}</span>
                </div>
                <button
                  v-if="!isDeleteSessionsConfirmationOpen"
                  class="sidebar-session-danger-open"
                  type="button"
                  @click="isDeleteSessionsConfirmationOpen = true; deleteAllSessionsError = ''"
                >
                  {{ t('Delete all sessions and threads') }}
                </button>
                <div v-else class="sidebar-session-danger-confirm">
                  <label for="delete-all-sessions-confirmation">
                    {{ t('Type DELETE ALL SESSIONS to confirm. Deletion is refused while any work or approval is active.') }}
                  </label>
                  <input
                    id="delete-all-sessions-confirmation"
                    v-model="deleteSessionsConfirmation"
                    type="text"
                    autocomplete="off"
                    spellcheck="false"
                    placeholder="DELETE ALL SESSIONS"
                    :disabled="isDeletingAllSessions"
                    @keydown.enter="onDeleteAllSessions"
                  >
                  <div class="sidebar-session-danger-actions">
                    <button
                      type="button"
                      :disabled="isDeletingAllSessions"
                      @click="isDeleteSessionsConfirmationOpen = false; deleteSessionsConfirmation = ''; deleteAllSessionsError = ''"
                    >{{ t('Cancel') }}</button>
                    <button
                      class="sidebar-session-danger-confirm-button"
                      type="button"
                      :disabled="isDeletingAllSessions || deleteSessionsConfirmation !== 'DELETE ALL SESSIONS'"
                      @click="onDeleteAllSessions"
                    >{{ isDeletingAllSessions ? t('Deleting safely…') : t('Permanently delete sessions') }}</button>
                  </div>
                </div>
                <div v-if="deleteAllSessionsError" class="sidebar-session-danger-error" role="alert">
    ...
    <template #content>
      ...
      >
        ...
        <ContentHeader :title="contentTitle" :accent="isSkillsRoute || isAutomationsRoute">
          ...
          <template #actions>
            <button
              class="content-header-connection"
            ...
            >
              ...
              <span class="content-header-connection-label">{{ connectionStateLabel }}</span>
              <span class="content-header-connection-refresh" aria-hidden="true">↻</span>
            </button>
            <ComposerDropdown
            ...
            <div class="content-grid content-grid-home">
              ...
              <div class="composer-with-queue">
                ...
                <ThreadComposer ref="homeThreadComposerRef" :active-thread-id="composerThreadContextId"
                  ...
                  :selection-evidence="selectedThreadSelectionEvidence"
...
function onConnectionStateEvent(event: Event): void {
  const custom = event as CustomEvent<{ state?: FrontierConnectionState }>
  const state = custom.detail?.state
  if (state && ['connecting', 'connected', 'reconnecting', 'fallback', 'offline'].includes(state)) {
...
const isDeleteSessionsConfirmationOpen = ref(false)
const deleteSessionsConfirmation = ref('')
const isDeletingAllSessions = ref(false)
const deleteAllSessionsError = ref('')
...
const MOBILE_RESUME_RELOAD_MIN_HIDDEN_MS = 5_000
...
async function onDeleteAllSessions(): Promise<void> {
  if (deleteSessionsConfirmation.value !== 'DELETE ALL SESSIONS' || isDeletingAllSessions.value) return
  isDeletingAllSessions.value = true
  deleteAllSessionsError.value = ''
  try {
    await deleteAllSessionsAndThreads(deleteSessionsConfirmation.value)
    window.location.assign('/')
  } catch (error) {
    deleteAllSessionsError.value = error instanceof Error ? error.message : t('Failed to delete sessions safely')
...
function onEditQueuedMessage(messageId: string): void {
  ...
  editingQueuedMessageState.value = selectedThreadId.value
    ...
    : null
  onSelectModel(message.modelId)
  onSelectReasoningEffort(message.reasoningEffort)
...
async function submitFirstMessageForNewThread(
  ...
  selection?: { modelId: string; reasoningEffort: ReasoningEffort | '' },
): Promise<void> {
  try {
    worktreeInitStatus.value = { phase: 'idle', title: '', message: '' }
...
.content-header-connection {
  @apply inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full border border-emerald-300/70 bg-emerald-50 px-2.5 text-xs font-semibold text-emerald-800 shadow-sm transition hover:bg-emerald-100 active:scale-[0.98];
}

.content-header-connection[data-state='connecting'],
.content-header-connection[data-state='reconnecting'],
.content-header-connection[data-state='fallback'] {
  @apply border-amber-300/70 bg-amber-50 text-amber-800 hover:bg-amber-100;
}

.content-header-connection[data-state='offline'] {
  @apply border-rose-300/70 bg-rose-50 text-rose-800 hover:bg-rose-100;
}

.content-header-connection-dot {
  @apply h-2 w-2 rounded-full bg-current;
  box-shadow: 0 0 0 3px rgb(16 185 129 / 12%);
}

.content-header-connection[data-state='connecting'] .content-header-connection-dot,
.content-header-connection[data-state='reconnecting'] .content-header-connection-dot,
.content-header-connection[data-state='fallback'] .content-header-connection-dot {
  opacity: 0.72;
}

.content-header-connection-refresh {
  @apply text-base leading-none;
}

:global(:root.dark) .content-header-connection {
  @apply border-emerald-700/70 bg-emerald-950/60 text-emerald-200 hover:bg-emerald-950;
}

:global(:root.dark) .content-header-connection[data-state='connecting'],
:global(:root.dark) .content-header-connection[data-state='reconnecting'],
:global(:root.dark) .content-header-connection[data-state='fallback'] {
  @apply border-amber-700/70 bg-amber-950/60 text-amber-200 hover:bg-amber-950;
}

:global(:root.dark) .content-header-connection[data-state='offline'] {
  @apply border-rose-700/70 bg-rose-950/60 text-rose-200 hover:bg-rose-950;
}

@media (max-width: 560px) {
  .content-header-connection {
    @apply h-9 min-w-9 justify-center px-2;
  }

  .content-header-connection-label {
    @apply sr-only;
  }
}
...
.sidebar-session-danger {
  @apply mx-2 rounded-xl border border-rose-300 bg-rose-50 p-3 text-rose-950;
}

.sidebar-session-danger-copy {
  @apply flex flex-col gap-1 text-xs leading-relaxed;
}

.sidebar-session-danger-copy strong {
  @apply text-sm;
}

.sidebar-session-danger-open {
  @apply mt-3 min-h-12 w-full rounded-lg bg-rose-700 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-60;
}

.sidebar-session-danger-confirm {
  @apply mt-3 flex flex-col gap-2 text-xs;
}

.sidebar-session-danger-confirm input {
  @apply w-full rounded-lg border border-rose-300 bg-white px-3 py-2 font-mono text-sm text-zinc-950 outline-none focus:border-rose-600 focus:ring-2 focus:ring-rose-200;
}

.sidebar-session-danger-actions {
  @apply grid grid-cols-2 gap-2;
}

.sidebar-session-danger-actions button {
  @apply min-h-10 rounded-lg border border-rose-300 bg-white px-2 py-2 font-semibold text-rose-900 disabled:cursor-not-allowed disabled:opacity-50;
}

.sidebar-session-danger-actions .sidebar-session-danger-confirm-button {
  @apply border-rose-700 bg-rose-700 text-white;
}

.sidebar-session-danger-error {
  @apply mt-2 rounded-lg bg-rose-100 px-3 py-2 text-xs font-medium text-rose-900;
}

:root.dark .sidebar-session-danger {
  @apply border-rose-900 bg-rose-950/40 text-rose-100;
}

:root.dark .sidebar-session-danger-confirm input,
:root.dark .sidebar-session-danger-actions button {
  @apply border-rose-800 bg-zinc-950 text-rose-100;
}
